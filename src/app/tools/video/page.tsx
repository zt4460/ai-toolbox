...
export default function VideoToolPage() {
  ...
  const getVideoCost = () => {
    let cost = 10; // 基础消耗
    if (resolution === '1080p') cost += 10;
    if (duration === '8') cost += 5;
    if (duration === '12') cost += 10;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.videoUrl) {
        ...
        setSuccessMessage(`视频生成成功！消耗 ${cost} 积分`);
        await refreshUser();
      } else {
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-red-500">本次消耗 {getVideoCost()} 积分</span>
            </div>
            <button 
              onClick={handleGenerate}
              disabled={!prompt.trim() || isGenerating}
